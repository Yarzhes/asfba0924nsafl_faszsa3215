...
async def main_loop():
    ...
    try:
        ...
        async for event in ws_client.start():
            ...
            logger.info(f"Processing closed kline for {kline.symbol}/{kline.timeframe}...")
            ...
            if ohlcv is None or len(ohlcv) < settings.features.warmup_periods:
                logger.debug(f"Not enough data for {kline.symbol}/{kline.timeframe} to compute features yet.")
                continue

            # --- 5a. OHLCV-based features ---
            ohlcv_features: dict = {}
            ohlcv_features.update(compute_trend(ohlcv, settings.features.trend.model_dump()))
            ohlcv_features.update(compute_momentum(ohlcv, settings.features.momentum.model_dump()))
            ohlcv_features.update(compute_volatility(ohlcv, settings.features.volatility.model_dump()))
            ohlcv_features.update(compute_volume_flow_features(ohlcv, **settings.features.volume_flow.model_dump()))
            ...
            ob_v2_features = compute_orderbook_features_v2(
                feature_store, kline.symbol, book_flip_states[kline.symbol], settings.features.orderbook_v2.model_dump()
            )
            cvd_features = compute_cvd_features(
                feature_store, kline.symbol, cvd_states[kline.symbol], settings.features.cvd.model_dump()
            )
            ...
            if not risk_result.passed:
                logger.info(f"Signal for {signal.symbol} blocked by risk filter: {risk_result.reason}")
                continue
            ...
            if sized_signal.decision != "NO_TRADE":
                ...
                if not settings.transport.dry_run:
                    ...
                    logger.info(f"DRY RUN: Telegram message would be sent:\n---\n{message}\n---")
